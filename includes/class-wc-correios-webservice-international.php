...
class WC_Correios_Webservice_International {
	...
	public function get_shipping() {
		...
		if ( 'yes' === $this->debug ) {
			$this->log->add( $this->id, 'Requesting BlueX WebServices: ' . $url );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( isset( $result->tipo_servico ) ) {
				if ( 'yes' === $this->debug ) {
					$this->log->add( $this->id, 'BlueX WebServices response: ' . print_r( $result, true ) );
				}
			...
			if ( 'yes' === $this->debug ) {
				$this->log->add( $this->id, 'Error accessing the BlueX WebServices: ' . print_r( $response, true ) );
			}
		}
